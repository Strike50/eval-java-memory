...
public class App {
    ...
    private static void restoreContacts() {
        ...
        try ( DirectoryStream<Path> ds = Files.newDirectoryStream( Paths.get( BOOK_BKP_DIR ), "*.ser" ) ) {
            ...
            //On duplique la génération d'itérateur car on ne peut pas revenir au début d'un itérateur
            DirectoryStream<Path> dsBis = Files.newDirectoryStream( Paths.get( BOOK_BKP_DIR ), "*.ser" );
            Iterator<Path> iteratorBis = dsBis.iterator();
            if (!iterator.hasNext()){
                System.out.println("*Il n'y a aucun fichier à restaurer.");
            }
            else {
                int i = 1;
                System.out.println("*Veuillez choisir un fichier parmis les suivants pour restauration :");
                ...
                if (response !=-1 && response <= i) {
                    i = 1;
                    for (; iteratorBis.hasNext(); ) {
                        Path path = iteratorBis.next();
                        if(i == response){
                            System.out.println("Restauration du fichier en cours : " + path.getFileName());
